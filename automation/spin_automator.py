...
class SpinAutomator:
    ...
    def _confirm_ready_for_ad(self) -> bool:
        """
        Guard rail antes de iniciar anuncio.

        O app exibe "Use all your spins before watching a video..." quando ainda
        existe spin pendente. Para preservar eficiencia, nunca clicamos no anuncio
        sem antes tentar SPIN e confirmar novamente que o contador esta sem spins.
        """
        self.log("Guarda anti-popup: verificando spins antes do anuncio...", "info")

        first_status = check_spins_status(self.device, self.device_profile)
        if first_status == "HAS_SPINS":
            self.log("Spins disponiveis detectados. Girando antes de assistir anuncio.", "success")
            return False

        spin_btn = find_spin_button(self.device)
        if spin_btn:
            self.log("Apertando SPIN preventivo antes de procurar anuncio.", "action")
            if not self.dry_run:
                spin_btn.click()
            self.last_action_time = time.time()
            self.wait(getattr(config, "PRE_AD_SPIN_CHECK_WAIT", 1.2))

            if check_and_dismiss_warning_popup(self.device):
                self.log("Popup de spins apareceu; anuncio bloqueado e fluxo voltara a girar.", "warning")
                self.wait(0.8)
                return False

            status_after_spin = check_spins_status(self.device, self.device_profile)
            if status_after_spin == "HAS_SPINS":
                self.log("SPIN preventivo encontrou spins disponiveis. Cancelando anuncio.", "success")
                return False
            if status_after_spin == "UNKNOWN":
                self.log("Nao foi possivel confirmar ausencia de spins apos SPIN. Evitando anuncio por seguranca.", "warning")
                return False

        self.wait(getattr(config, "PRE_AD_CONFIRM_WAIT", 1.0))
        final_status = check_spins_status(self.device, self.device_profile)
        if final_status != "NO_SPINS":
            self.log(f"Anuncio bloqueado: estado de spins ainda e {final_status}.", "warning")
            return False

        self.log("Sem spins confirmado apos tentativa preventiva. Anuncio liberado.", "info")
        return True
            ...
            if is_main_screen(self.device):
                ...
                if check_and_dismiss_warning_popup(self.device):
                    ...
                    if spin_btn:
                         ...
                         # Se o aviso persistir, reiniciamos sem abrir anuncio.
                         if check_and_dismiss_warning_popup(self.device):
                             self.log("Aviso persistente mesmo apos SPIN. Reiniciando sem abrir anuncio.", "error")
                ...
                if status == "HAS_SPINS":
                    ...
                    if not self._confirm_ready_for_ad():
                        continue
